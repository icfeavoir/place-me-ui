...
module.exports = {
  getAll(req, res) {
    Group.scope(['defaultScope', 'orderByName'])
      .findAll()
      .then((groups) => {
  ...
  getById(req, res) {
    this.getGroup(req.params.groupId).then((group) => {
  ...
  getByEventPlanId(req, res) {
    ...
    Group.scope(['defaultScope', 'orderByNumber'])
      .findAll({ where: { event_plan_id: eventPlanId } })
      .then((groups) => {
        this._handleResponse(groups, res);
      })
      .catch((e) => {
  ...
  create(req, res) {
    ...
    if (result === true) {
      ...
      this.findOrCreateConstraint(params.constraint_name).then(
        ([constraint, created]) => {
  ...
  checkData(params) {
    ...
    if (!params.constraint_name) {
      ...
      if (
        ...
        params.constraint_number > params.number
      ) {
